...
function registerIpcHandlers() {
  ...
  ipcMain.handle('vpn:connect', async (_, countryCode: string) => {
    ...
    // If manual proxy is active, clear it first (VPN takes over)
  ...
  ipcMain.handle('agent:start-task', async (_, taskId: string) => {
    ...
    // Pass:
    //   • sidebar-width getter so the view is positioned correctly
    //   • fallback proxy getter so the agent inherits VPN / manual proxy
    ae.startTask(
      taskId,
      win,
      () => bm.getSidebarWidth(),
